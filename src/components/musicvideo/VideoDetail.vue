    ...
    <div class="mv-right">
      ...
      <div style="display: flex; justifycontent: space-between">
        <span style="fontsize: 13px; color: #888"
          >发布时间:{{ topMenu.publishTime | showDate }}</span
        >
        <span style="fontsize: 13px; color: #888"
          >播放次数:{{
            topMenu.playTime > 10000
              ? (topMenu.playTime / 10000).toFixed(0) + "万"
              : topMenu.playTime
          }}次</span
        >
      </div>
...
import MvComment from "@/components/singerlist/MvComment";
import VideoCard from "@/components/musicvideo/VideoCard";
export default {
  name: "SingerMvVideo",
  components: {
    TitleHeader,
