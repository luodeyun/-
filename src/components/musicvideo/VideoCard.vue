  ...
  <div class="mvsimicard">
    <div class="simi-img" @click="togMv(simis.vid)">
      ...
      <div class="r-top">
        ...
        {{
          simis.playTime > 10000
            ? (simis.playTime / 10000).toFixed(0) + "万"
            : simis.playTime
        }}
      </div>
    </div>
    <div class="simi-content">
      ...
      >
        <i v-for="(item, index) in simis.creator" :key="item.userId">{{
          simis.creator.length === index + 1
            ? item.userName
            : item.userName + "/"
        }}</i>
      </div>
      <span style="fontsize: 13px; color: #888" v-else>{{
        simis.creator[0].userName
      }}</span>
    </div>
  </div>
</template>
