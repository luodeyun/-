        ...
        <el-tab-pane label="全部" name="first">
          ...
          >
            ...
            <template v-slot:tablesinger="poces">
              <span v-if="poces.item.length === 1">{{
                poces.item[0].name
              }}</span>
              <span v-else>
                <i v-for="(item, index) in poces.item" :key="item.id">{{
                  index + 1 === poces.item.length ? item.name : item.name + "/"
                }}</i>
              </span>
            </template>
          </SingerAlbum>
...
<script>
import SingerAlbum from "@/components/singerlist/SingerAlbum";
import NewAlbumList from "@/components/newsongs/NewAlbumList.vue";
